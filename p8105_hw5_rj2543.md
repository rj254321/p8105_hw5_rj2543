...
map(patient_tidy %>% distinct(id) %>% pull(), plot_patient) # apply functions to all distinct subjects
```
...
full_join(nhomicide, nunsolved) # combine above information in one dataframe without omit
```
